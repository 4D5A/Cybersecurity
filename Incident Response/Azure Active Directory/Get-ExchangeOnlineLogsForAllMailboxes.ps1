...
{  
    ...
    $CurrMessages = Get-MessageTrace -PageSize 5000 -Page $Page -StartDate (Get-Date).Adddays(-10) -EndDate (Get-Date) | Select MessageId, Organization, Received, SenderAddress, RecipientAddress, Subject, Status, FromIP, ToIP, PSComputerName, RunspaceId, MessageTraceId
    $Page++  
